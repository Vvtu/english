...
const dictionaries: dictSet = {
  ...
  dictionary2: [
    ...
    {
      rus: 'В этом есть много истории, но однажды мы сможем. Давай держись =)',
    ...
    {
      rus:
        'Старайтесь использовать кортеж локально. Для долгоживущих и часто передаваемых структур данных предпочтительнее использовать записи с именованными полями.',
      eng:
    ...
    {
      rus:
        "Существование кортежей может показаться странным для тех, кто приходит из нетипизированных языков. 'Почему бы просто не использовать список/массив?",
      eng:
  ...
  oldDictionary2: [
    ...
    {
      rus:
        'Существует специальный синтаксис для строк, который позволяет делать многострочные строки, как и раньше, без специальных escape символов, хуки для специальных предпроцессоров. {|...|}  {js|...|js}',
      eng:
        "There's a special syntax for string that allows multiline string just like before, no special character escaping, hooks for special pre-processors. {|...|}  {js|...|js}",
    },
    {
      rus:
  ...
  oldDictionary2: [
    ...
    {
      rus: 'Далее нам нужно определить, какой компонент мутирует или владеет этим состоянием.',
